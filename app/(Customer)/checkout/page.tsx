...
export default function CheckoutPage() {
  ...
  // Debounced copy of deliveryAddress. Recalculating distance/time on every
  // keystroke was causing wrong results: each keystroke fired its own
  // forwardGeocode() call, and slower/older responses could resolve AFTER
  // newer ones and overwrite the correct distance with a stale one. We now
  // wait for the user to pause typing before geocoding.
  ...
  // When the user detects their location via GPS, we already have exact
  // coordinates. Re-running them through forwardGeocode(reverseGeocodedAddress)
  // could snap to slightly different coordinates than the real GPS fix,
  // producing a slightly-wrong distance. We keep the GPS coords here and use
  // them directly instead of re-geocoding, as long as the address hasn't
  // since been hand-edited by the user.
  ...
  // Monotonically increasing id used to ignore stale async responses: if a
  // newer distance calculation has started, any older one that resolves
  // later is discarded instead of overwriting the newer (correct) result.
  ...
  useEffect(() => {
    ...
    const calculateDistance = async () => {
      ...
      if (addressSourceRef.current === "gps" && gpsCoordsRef.current) {
        // Address on screen is exactly what GPS detected — use those coords directly.
      ...
      // Bail out if a newer request has started, or the component unmounted,
      // while we were awaiting the geocode/route lookup above.
      ...
      if (coords && branchCoords) {
        ...
        if (mbRoute) {
          setDistanceKm(mbRoute.distanceKm);
          setRouteInfo({
