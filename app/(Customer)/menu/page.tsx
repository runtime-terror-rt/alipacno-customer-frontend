        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden pt-6">
          ...
          <div className="flex flex-col">
            {categories.map((cat: any, i: number) => {
              ...
              const hasSvgIcon = cat.icon && clean(cat.icon).includes("<svg");
              const iconSvg = hasSvgIcon ? clean(cat.icon) : null;
              const imageSrc = clean(cat.image_url);
              ...
              return (
                ...
                >
                  <div
                    className={`w-[22px] h-[22px] mr-4 flex items-center justify-center ${isActive ? "text-[#F9671A]" : "text-zinc-500 group-hover:text-zinc-400"
                      }`}
                  >
                    {iconSvg ? (
                      <span
                      ...
                      <img
                        src={imageSrc}
                        alt={cat.name}
                        className={`w-full h-full object-contain ${isActive ? "" : "opacity-70 group-hover:opacity-100"
                          }`}
                      />
                    ) : null}
                  </div>
